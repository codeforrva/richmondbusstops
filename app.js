...
app.post('/find_address', function(req, res){
	var app_res = res,
			app_req = req,
			myLocation, nearestBusStop, nearestBusStopAddress,
			routes_coordinates = [],
	...
	var geoAddressData = '',
			geocodeAPI = "http://www.datasciencetoolkit.org/maps/api/geocode/json?sensor=false&address=" + encodeURIComponent(address),
			geoAddress = http.get(geocodeAPI, function(res) {
		...
		res.on('end', function() {
			...
			myLocation = [geoAddressData.results[0].geometry.location.lat, geoAddressData.results[0].geometry.location.lng];
